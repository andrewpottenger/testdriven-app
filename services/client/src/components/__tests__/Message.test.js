...
describe('When given a success message', () => {
    ...
    it(`Message renders properly`, () => {
        ...
        expect(span.get(1).props.children[1]).toContain(messageSuccessProps.messageName);
...
describe('When given a danger message', () => {
    ...
    it(`Message renders properly`, () => {
        ...
        const span = wrapper.find('span');
        expect(span.length).toBe(2);
        expect(span.get(1).props.children[1]).toContain(messageDangerProps.messageName);
        const button = wrapper.find('button');
        expect(button.length).toBe(1);
        expect(removeMessage).toHaveBeenCalledTimes(0);
        button.simulate('click');
